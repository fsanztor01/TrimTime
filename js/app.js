...
function showServiceDetails(service) {
    ...
    if (modal && content) {
        ...
        const toggleBtn = document.getElementById('toggleServiceDesc');
        if (toggleBtn) {
            toggleBtn.addEventListener('click', () => {
                const shortDesc = content.querySelector('.service-desc-short');
                const fullDesc = content.querySelector('.service-desc-full');
                if (shortDesc && fullDesc) {
                    if (fullDesc.style.display === 'none') {
                        fullDesc.style.display = 'block';
                        shortDesc.style.display = 'none';
                        toggleBtn.textContent = translations[state.currentLanguage]['service.readLess'] || 'Read Less';
                    } else {
...
window.updateAppointmentStatus = async function (appointmentId, newStatus) {
    try {
        ...
        if (result.success) {
            ...
            // Update appointment card visually without reload
            const appointmentCards = document.querySelectorAll(`[data-appointment-id="${appointmentId}"]`);
            appointmentCards.forEach(card => {
                const statusEl = card.querySelector('.appointment-status');
                if (statusEl) {
                    statusEl.className = `appointment-status status-${newStatus}`;
                    statusEl.textContent = translations[state.currentLanguage][`status.${newStatus}`] || newStatus;
        ...
        submitBtn.onclick = async () => {
            try {
                ...
                if (result.success) {
                    ...
                    showToast(translations[state.currentLanguage]['rating.thankYou'] || 'Thank you for your rating!', 'success');
                    
                    // Show thank you modal/notification
                    const thankYouModal = document.createElement('div');
                    thankYouModal.className = 'thank-you-modal';
                    thankYouModal.innerHTML = `
                        <div class="thank-you-content">
                            <div class="thank-you-icon">✨</div>
                            <h3>${translations[state.currentLanguage]['rating.thankYou'] || 'Thank You!'}</h3>
                            <p>${translations[state.currentLanguage]['rating.appreciate'] || 'We appreciate your feedback!'}</p>
