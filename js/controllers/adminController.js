...
export class AdminController {
    static setupEventListeners(state, elements) {
        ...
        if (elements.filterAppointments) {
            elements.filterAppointments.addEventListener('click', () => AdminController.showFilterModal(state, elements));
        }
        ...
        if (refreshMessagesBtn) {
            refreshMessagesBtn.addEventListener('click', () => {
                AdminController.renderAdminMessages(state, elements);
            });
        }
...
`;
    ...
    static async renderAdminAppointments(state, elements) {
        ...
        try {
            ...
            if (appointmentsResult.success && servicesResult.success && barbersResult.success && usersResult.success) {
                let appointments = appointmentsResult.data.filter(a => !a.deleted);
                const services = servicesResult.data;
                const barbers = barbersResult.data;
                const users = usersResult.data;

                // Apply filters if they exist
                if (state.appointmentFilters) {
                    ...
                    if (filters.serviceId) {
                        appointments = appointments.filter(a => a.service_id === filters.serviceId);
                    }
                    if (filters.barberId) {
                        appointments = appointments.filter(a => a.employee_id === filters.barberId);
                    }
                    if (filters.status) {
                ...
                // Show/hide active filters badge
                const badge = document.getElementById('activeFiltersBadge');
                if (badge) {
                    if (state.appointmentFilters && Object.values(state.appointmentFilters).some(v => v !== null && v !== '')) {
                        badge.style.display = 'flex';
                    } else {
    ...
    static showAddServiceModal(state, elements) {
        ...
        <form id="addServiceForm">
            ...
            <div class="form-group">
                ...
                <small>Upload a representative image for this service</small>
                <div id="serviceImagePreview" style="margin-top: 10px; display: none;">
                    <img id="servicePreviewImg" src="" alt="Preview" style="max-width: 200px; max-height: 150px; border-radius: 5px;">
                </div>
            </div>
            <div class="form-group">
    ...
    `;
        ...
        if (serviceImageInput) {
            serviceImageInput.addEventListener('change', (e) => {
                ...
                if (file && file.type.startsWith('image/')) {
                    ...
                    reader.onload = (event) => {
                        servicePreviewImg.src = event.target.result;
        ...
        document.getElementById('addServiceForm').addEventListener('submit', async (e) => {
            ...
            let imageData = `https://picsum.photos/seed/${generateId()}/300/200.jpg`;
    ...
    static showAddBarberModal(state, elements) {
        ...
        content.innerHTML = `
            ...
            <form id="addBarberForm">
                ...
                <div class="form-group">
                    ...
                    <div id="barberPhotoPreview" style="margin-top: 10px; display: none;">
                        <img id="barberPreviewImg" src="" alt="Preview" style="max-width: 150px; max-height: 150px; border-radius: 50%; border: 2px solid var(--primary-color);">
                    </div>
                </div>
                <div class="form-group">
                    <label class="checkbox-label">
        ...
        const barberPhotoInput = document.getElementById('barberPhoto');
        const barberPhotoPreview = document.getElementById('barberPhotoPreview');
        const barberPreviewImg = document.getElementById('barberPreviewImg');
            ...
            barberPhotoInput.addEventListener('change', (e) => {
                ...
                if (file && file.type.startsWith('image/')) {
                    ...
                    reader.onload = (event) => {
                        ...
                        barberPhotoPreview.style.display = 'block';
                    };
                    reader.readAsDataURL(file);
                }
            });
        ...
        document.getElementById('addBarberForm').addEventListener('submit', async (e) => {
            ...
            let photoData = `https://picsum.photos/seed/${generateId()}/200/200.jpg`;
    ...
    static async showFilterModal(state, elements) {
        const modal = document.getElementById('filterModal');
        if (!modal) return;

        // Get services, barbers, and users for filter options
        const servicesResult = await DatabaseService.getServices(true);
        const barbersResult = await DatabaseService.getBarbers(true);
        const usersResult = await DatabaseService.getUsers();
        ...
        if (serviceFilter && servicesResult.success) {
            ...
            servicesResult.data.forEach(service => {
                ...
                if (service.id === currentValue) option.selected = true;
        ...
        if (barberFilter && barbersResult.success) {
            const currentValue = barberFilter.value;
            barberFilter.innerHTML = '<option value="">All barbers</option>';
            barbersResult.data.forEach(barber => {
                const option = document.createElement('option');
                option.value = barber.id;
                option.textContent = barber.name;
                if (barber.id === currentValue) option.selected = true;
        ...
        if (clientFilter && usersResult.success) {
        ...
        if (state.appointmentFilters) {
            if (state.appointmentFilters.dateFrom) {
                document.getElementById('filterDateFrom').value = state.appointmentFilters.dateFrom;
            }
            if (state.appointmentFilters.dateTo) {
                document.getElementById('filterDateTo').value = state.appointmentFilters.dateTo;
            }
            if (state.appointmentFilters.serviceId) {
                document.getElementById('filterService').value = state.appointmentFilters.serviceId;
            }
            if (state.appointmentFilters.barberId) {
                document.getElementById('filterBarber').value = state.appointmentFilters.barberId;
            }
            if (state.appointmentFilters.status) {
                document.getElementById('filterStatus').value = state.appointmentFilters.status;
            }
            if (state.appointmentFilters.clientId) {
                document.getElementById('filterClient').value = state.appointmentFilters.clientId;
            }
        }
        ...
        if (filterForm) {
            filterForm.onsubmit = (e) => {
                e.preventDefault();
                AdminController.applyFilters(state, elements);
            };
        }
        ...
        if (clearFiltersBtn) {
            clearFiltersBtn.onclick = () => {
                AdminController.clearFilters(state, elements);
            };
        }
    }

    static applyFilters(state, elements) {
        // Get filter values
        const filters = {
            dateFrom: document.getElementById('filterDateFrom').value || null,
            dateTo: document.getElementById('filterDateTo').value || null,
            serviceId: document.getElementById('filterService').value || null,
            barberId: document.getElementById('filterBarber').value || null,
        ...
        state.appointmentFilters = filters;
        ...
        // Re-render appointments with filters
        AdminController.renderAdminAppointments(state, elements);
    ...
    static clearFilters(state, elements) {
        // Clear filter inputs
        ...
        // Re-render appointments without filters
        AdminController.renderAdminAppointments(state, elements);
    ...
    // =============== MESSAGES ===============
    static async renderAdminMessages(state, elements) {
        showLoading(true);
        ...
        try {
            ...
            if (result.success && result.data.length > 0) {
                result.data.forEach(message => {
                    ...
                    messageCard.className = `admin-card message-card ${!message.read ? 'unread' : ''}`;
                    ...
                    messageCard.innerHTML = `
                        <div class="message-header">
                            <div class="message-info">
                                ...
                                <div class="message-meta">
                                    ...
                                    <span class="message-email">📧 ${message.userEmail || 'No email'}</span>
                        ...
                        <div class="message-actions">
                            ${!message.read ? `<button class="btn btn-secondary" onclick="markMessageAsRead('${message.id}')">Mark as Read</button>` : ''}
                        </div>
                    `;
